...
#define CHECK(label) do{__asm__ volatile ("" ::: "memory");\
		if(unlikely(dty->dirtyNphase != gclock)) \
		{dty->dirtyNphase=gclock; goto label;}} while(0)
//#define FENCE_CHECK(label) if(unlikely(CAS(&dty->dirty, 1, 0))) goto label;
#define FENCE_CHECK(label) do{__asm__ volatile ("" ::: "memory");\
		if(unlikely(__sync_lock_test_and_set(&dty->dirtyNphase, gclock) != gclock)) \
...
start:
	while (TRUE) {
		...
		//create entry
		Entry *newEntry = (Entry*)lf_palloc(sizeof(Entry));
